...
export default function EditKinkun() {
  //เอาข้อมูลที่ส่งต่อท้ายลิงค์ผ่าน useParams มาเก็บในตัวแปร
  const { id } = useParams();

  useEffect(() => {
    //โค้ดที่จะให้ทํางานเมื่อมี Effect เกิดขึ้นกับ Component
    //ดึงข้อมูลการกินหนึ่งๆ จาก Supabase
    const fetchKinkun = async () => {
      ...
      if (error) {
        ...
        setFoodNile(data.food_image_url);
      }
    };
  ...
  const handleSaveUpdateClick = async (e) => {
    ...
    //บันทึกเเก้ไขข้อมูลไปที่ table บน Supabase
    ...
    successAlert("บันทึกเเก้ไขการกินเรียบร้อยแล้ว");
